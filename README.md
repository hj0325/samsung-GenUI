...
```mermaid
flowchart TD
  user["User<br/>scenario text"] --> stream["/api/pipeline/full/stream<br/>(SSE)"]
  stream --> s1["1+2 · Interpret + Normalize<br/><i>gpt-5.4-mini</i>"]
  s1 --> par{"Promise.all"}
  par --> s3["3 · Select<br/><i>gpt-5.4 + RAG shortlist</i>"]
  par --> s35["3.5 · Content Bag<br/><i>gpt-5.4-mini · parallel</i>"]
  s3 --> swap["applyContentSwap<br/>fills empty / dup slots"]
  s35 --> swap
  swap --> s4["4 · Compose Layout<br/><i>gpt-5.4</i>"]
  s4 --> s5["5 · Validate<br/>(plan + layout violations)"]
  s5 --> s7["7 · Explain<br/><i>gpt-5.4-mini</i><br/>(skipped in fastMode)"]
  s7 --> render["Renderer<br/>scenes.js + surface-layout.js"]
  render --> canvas["Phone frame<br/>(Galaxy S26)"]

  classDef llm fill:#0381FE22,stroke:#0381FE,color:#fff
  classDef proc fill:#0FCF6E22,stroke:#0FCF6E,color:#fff
  classDef io fill:#F5A62322,stroke:#F5A623,color:#fff
  class s1,s3,s35,s4,s7 llm
  class swap,s5,render proc
  class user,stream,canvas io
```

Every box that's an LLM call is one OpenAI request. Stages 3 and 3.5
fire **in parallel** so the content bag adds zero critical-path latency.
...
mechanical extraction lives on `gpt-5.4-mini`. Both are configurable in
...
sequenceDiagram
  ...
  Pipe->>Pipe: applyContentSwap<br/>fill empty / dup slots from bag
...
selector prompt is wasteful and pollutes the LLM's choice space with
components that don't even have a renderer.

```mermaid
flowchart LR
  A[Registry · 92 components] -->|filter to renderable| B[52 renderable IDs]
  B -->|cosine top-K against<br/>scenario embedding| C[Top 30]
  C -->|+ surface-mandatory<br/>(status-bar, header)| D[31–32 final shortlist]
  D --> sel["Stage 3 selector prompt"]

  classDef bad fill:#EF444422,stroke:#EF4444
  classDef good fill:#0FCF6E22,stroke:#0FCF6E
  A:::bad
  D:::good
```

The 40 components dropped by the renderable filter are things like
`card.subheading` / `card.menu-item-body` / `dialog.browser-top-bar` —
they exist in the registry but the renderer (`scenes.js` +
`templates.js` + `surface-layout.js`) has no template for them, so they
fall through to a `(no template registered)` placeholder card and trip
multiple validators. Filtering them out at the embedding stage means
the selector physically cannot pick a non-renderable id.

**Source of truth for the renderable set:** four maps that have to stay
in sync —
...
The union (52 IDs) lives in `pipeline.js` as `RENDERABLE_COMPONENT_IDS`.
Adding a new id to any of those four maps requires adding it here too.
...
tofu …" three times in a row. We catch that at four levels:

```mermaid
flowchart TD
  raw["Raw LLM picks"] --> t1{"PRIMARY<br/>componentType + normalized label"}
  t1 -- match --> drop1[drop]
  t1 -- pass --> t2{"SECONDARY<br/>componentType + label + value"}
  t2 -- match --> drop2[drop]
  t2 -- pass --> t3{"TERTIARY<br/>alphanumeric-only<br/>(componentType + label + value)"}
  t3 -- match --> drop3[drop]
  t3 -- pass --> t4{"QUATERNARY<br/>cross-type label OR value<br/>(catches btn-contained 'Buy' + chip-row 'Buy')"}
  t4 -- match --> drop4[drop]
  t4 -- pass --> typecap{"TYPE_CAP<br/>≤ 3 of any componentType"}
  typecap -- over --> drop5[drop]
  typecap -- under --> survive[survive]

  classDef drop fill:#EF444422,stroke:#EF4444
  class drop1,drop2,drop3,drop4,drop5 drop
```

Each tier loosens the matching criterion. Most duplicates die at PRIMARY;
the cross-type tier is the safety net for `btn-contained "Coupang cart"`
+ `action_chip_row "Coupang cart"` style collisions where the label is
identical but the componentType isn't.

The selector prompt also bakes in **diversity rules** that nudge the LLM
upstream: max 2 of the same componentType, every label must be unique,
prefer variety, and `input_summary_card` is reserved for form summaries
(not a generic content card).

---

## Per-slot content resolution (the bug that wasn't a dedup bug)

A subtle renderer trap: when the plan has 3 distinct `input_summary_card`
entries with different content (e.g. "Cooking session", "Recipe",
"Step 3 of 7"), the old renderer keyed `contentByType` by componentType
only — so `Map.set('input_summary_card', …)` overwrote each previous
entry, and all 3 layout children rendered with the **last** entry's
content. Result: the screen looked like 3 identical step cards.

The fix is a 3-tier resolver:

```mermaid
flowchart LR
  child["layout child<br/>{componentId, slot}"] --> r1{"contentBySlot.get(slot)"}
  r1 -- hit --> done["use this content"]
  r1 -- miss --> r2{"contentByTypeQueue.shift()<br/>per type, plan order"}
  r2 -- hit --> done
  r2 -- miss --> r3["contentByType<br/>(legacy last-wins)"]
  r3 --> done

  classDef ok fill:#0FCF6E22,stroke:#0FCF6E
  class done ok
```

Distinct slots get their own content; identical-type-different-content
plan entries are handed out in plan order via the queue.

---

## Layout composer post-fixes

The composer LLM produces the layout, then a chain of deterministic
post-processors adjust it before validation:

| Post-fix | What it does |
|---|---|
| `chrome-role correction` | LLM sometimes tags content components as `role: "chrome"` — demoted back to subject/state/context |
...
| `role-based child reorder` | Each group is sorted subject → state → context → action → feedback |
...
**Chrome exemption** in the validators — `validateLayoutOverflow` skips
width / visibleChildren checks for any child whose `role === 'chrome'`
or whose parent group is `role === 'chrome'`. Chrome is full-width by
design and doesn't compete for the user's attention budget, so flagging
it was a false positive that produced 1–2 noise violations every run.
...
```mermaid
flowchart TD
  themes["figma-refs/themes.json<br/>(default · minimal · vibrant · mono · custom)"]
  active["_active id"]
  api["/api/themes (GET / POST)<br/>/api/themes/active (POST)"]
  customize["/customize · two-pane editor"]
  root[":root inline<br/>style.setProperty(--var)"]
  vars["CSS variables<br/>--page-bg, --card-radius,<br/>--card-weather-accent, …"]
  consumer["Card kinds<br/>(weather / calendar / reminder /<br/>message / eta / input / now-bar)"]
  bc["BroadcastChannel<br/>'oneui-themes'"]

  themes --> api
  active --> api
  api --> customize
  customize -- live preview --> root
  customize -- Save --> api
  customize -. theme-saved / theme-active-changed .-> bc
  bc -. genui topbar picker .-> root
  root --> vars
  vars --> consumer
  consumer --> canvas[Phone canvas]

  classDef store fill:#F5A62322,stroke:#F5A623
  classDef ui fill:#0381FE22,stroke:#0381FE
  class themes,active store
  class customize,canvas ui
```

**Live preview is real-time.** `_writeVar(key, value)` in the customizer
writes to `document.documentElement.style.setProperty` on every input
event. CSS variables propagate immediately to all consumers; the
preview grid re-renders on the same tick. The **Save** button is the
only persistence step — it POSTs the new theme and atomically
broadcasts a `theme-active-changed` to other open windows so the GenUI
main app's topbar picker updates without a manual refresh.

Theme transitions across the page chrome (sidebar, topbar, panels,
canvas) animate over 280ms `cubic-bezier(0.2, 0, 0, 1)` for color /
background / border / shadow — limited to non-layout properties so
there's no reflow. `prefers-reduced-motion` skips the animation.

---

## Self-improving loop

The system has 20 test scenarios (15 train + 5 holdout) in
`figma-refs/test_scenarios.json`. The improvement engine runs the
suite, asks an LLM to extract patterns from violations, trials a
candidate rule against the scenarios, and persists rules that
genuinely move the violation count down on holdouts.

```mermaid
flowchart LR
  suite["Test suite<br/>20 scenarios"] --> run["Run pipeline<br/>per scenario"]
  run --> agg["Aggregate violations<br/>by ruleId × scenario"]
  agg --> ext["Pattern extraction<br/>(LLM analyzes failure clusters)"]
  ext --> cand["Candidate rule<br/>e.g. 'For glanceable +<br/>cooking, prefer reminder<br/>over input_summary_card'"]
  cand --> trial["Trial<br/>(re-run suite WITH the rule)"]
  trial --> delta{"holdout violations<br/>improved?"}
  delta -- yes --> persist["Persist to evolve.md<br/>+ rehydrate at boot"]
  delta -- no --> reject["Reject"]
  persist --> suite
...
`data/improvement_history/`. The dashboard lives at `/improve`.
...
| `/api/agent/health` | GET | Server health + model name (used by Local↔AI mode toggle) |
...
| **Client (renderer)** | `app/scenes.js` | `renderPipelineResponse` orchestrator, per-slot content resolver, focus-block / now-bar / action-row adapters, kind parsers (weather, calendar, reminder, message, eta, input, navigation) |
...
Boot banner reflects which models are routed where, whether RAG is on,
how many scenarios are loaded, and how many learned rules were
rehydrated:
