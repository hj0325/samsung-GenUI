...
const { loadEnvFileOnce } = require('../config');
...
if (!OPENAI_API_KEY) {
  console.warn('\x1b[33m[WARN]\x1b[0m OPENAI_API_KEY not set in .env file');
}
...
const designMdPath = path.join(ROOT_DIR, 'DESIGN.md');
const genuiMdPath = path.join(ROOT_DIR, 'GENUI-PRINCIPLES.md');
const orchMdPath = path.join(ROOT_DIR, 'ORCHESTRATION.md');
const evolveMdPath = path.join(ROOT_DIR, 'evolve.md');
...
const designRulesPath = path.join(ROOT_DIR, 'figma-refs/design_rules.json');
const globalRulesPath = path.join(ROOT_DIR, 'figma-refs/global_rules.json');
...
async function handleLegacyApiRequest(req, res) {
  ...
  if (url === '/api/themes' && req.method === 'GET') {
    try {
      sendJSON(res, 200, themeStore.readThemes());
  ...
  if (url === '/api/themes/active' && req.method === 'POST') {
    try {
      ...
      const result = themeStore.setActiveTheme(id);
  ...
  if (url === '/api/themes' && req.method === 'POST') {
    try {
      ...
      const saved = themeStore.saveTheme(theme, { replace: !!body.replace });
      console.log('[themes] saved theme: ' + theme.id);
      sendJSON(res, 200, saved);
    } catch (e) {
      const status = e && e.code === 'THEME_EXISTS' ? 409 : 500;
  ...
  if (url === '/api/improve/history' && req.method === 'GET') {
    sendJSON(res, 200, { reports: historyStore.listReports() });
    return;
  }
  ...
  if (url === '/api/improve/extract' && req.method === 'POST') {
    try {
      ...
      if (!reportFilename) {
        const all = historyStore.listReports();
        if (all.length === 0) {
      ...
      const report = historyStore.readReport(reportFilename);
      if (!report) {
  ...
  if (url === '/api/improve/cycle' && req.method === 'POST') {
    try {
      ...
      if (body.sourceReport) {
        baselineReport = historyStore.readReport(body.sourceReport);
        if (!baselineReport) {
  ...
  if (url.startsWith('/api/improve/history/') && req.method === 'GET') {
    try {
      ...
      const report = historyStore.readReport(fname);
      if (!report) {
  ...
  const resolved  = path.resolve(path.join(ROOT_DIR, requested));
  if (resolved !== path.resolve(ROOT_DIR) && !(resolved + path.sep).startsWith(SAFE_ROOT)) {
...
function startLegacyApiServer() {
  if (!OPENAI_API_KEY) {
    throw new Error('OPENAI_API_KEY not set in .env file');
  }
  const server = http.createServer((req, res) => handleLegacyApiRequest(req, res));
...
if (require.main === module) {
  startLegacyApiServer();
}

function getAgentHealthPayload() {
  EVOLVE_CONSTRAINTS = loadEvolveConstraints();
  return {
    status: 'ok',
    model: OPENAI_MODEL,
    mode: 'surface-grammar',
    fallbacks: normalizer.getFallbackStats(),
    designKB: {
      mode: 'surface-grammar',
      designSections: Object.keys(DESIGN_SECTIONS).length,
      genuiSections: Object.keys(GENUI_SECTIONS).length,
